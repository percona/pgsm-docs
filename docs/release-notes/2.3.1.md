...
- Fixed a memory consumption issue that could lead to an assertion failure (`list_length(lentries) <= max_stack_depth`) and cause server crashes in some workloads.

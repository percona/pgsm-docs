...
Sets the maximum length of a query string (in bytes) that `pg_stat_monitor` will track. Queries longer than this value are truncated to the specified length, ensuring truncation does not split multi-byte UTF-8 characters. This helps avoid excessive shared memory usage.
